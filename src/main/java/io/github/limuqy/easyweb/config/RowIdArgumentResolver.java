        ...
        if (Collection.class.isAssignableFrom(type)) {
            ...
            if (text.startsWith("{")) {
                ...
                if (map.get(name) instanceof List<?>) {
                    list = CastUtils.cast(map.get(name));
                } else {
                    throw new RowIdException(HttpStatus.BAD_REQUEST);
                }
            } else {
            ...
            return list.stream().map(id -> RowIdUtil.decryptRowId(id, tableName)).collect(Collectors.toList());
            ...
            return JsonUtil.parseArray(text, String.class)
                    .stream()
                    .map(id -> RowIdUtil.decryptRowId(id, tableName)).toArray(Long[]::new);
        }
        return null;
    }
    ...
    private Object getParameterValue(Class<?> type, String[] values, RowId rowId) {
        ...
        if (Collection.class.isAssignableFrom(type)) {
            ...
            for (String value : values) {
                if (value.contains(",")) {
                    strList.addAll(Arrays.stream(value.split(",")).collect(Collectors.toList()));
                } else {
                    strList.add(value);
                }
            }
            return strList.stream().map(id -> RowIdUtil.decryptRowId(id, tableName)).collect(Collectors.toList());
        } else if (String.class.equals(type)) {
            return String.valueOf(RowIdUtil.decryptRowId(values[0], tableName));
        } else if (Number.class.isAssignableFrom(type)) {
            return RowIdUtil.decryptRowId(values[0], tableName);
        } else if (type.isArray()) {
            return Arrays.stream(values).map(id -> RowIdUtil.decryptRowId(id, tableName)).toArray(Long[]::new);
        }
        return null;
    }
